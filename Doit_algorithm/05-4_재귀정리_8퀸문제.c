...
// 한 행(row) 에 퀸을 하나씩 놓을 때, 이전에 놓은 퀸과 서로 공격이 되는지 검사하는 함수
...
// -> (같은 열이거나 같은 대각선일 경우, queenChk return 0) 다른 colPass 열 에서 퀸을 놓을 수 있는지 continue
...
void placeQueen(int* col, int* cnt, int row, int N) {
    if (row == N) { // 퀸 N 행에 놓았으면 cnt(경우의 수) 한 개 추가함!
        (*cnt)++;
        return;
    }
...
int main() {
    int* col = (int*)malloc(sizeof(int) * 8); // 각 행마다 퀸이 놓일 8 열의 공간 확보
    int cnt = 0; // 8 개의 queen 이 주어졌을 때 퀸이 위치할 수 있는 모든 경우의 개수

    placeQueen(col, &cnt, 0, 8); // 0번째 행부터 퀸의 위치 찾기.
    printf("8 * 8퀸 문제의 가능한 퀸 위치 모든 경우의 수 : %d\n", cnt);
    return 0;
}
